...
class AdminReorderMiddleware(MiddlewareMixin):
  ...
  def init_config(self, request, app_list):
    ...
    if not isinstance(self.config, (tuple, list)):
      raise ImproperlyConfigured('ADMIN_REORDER config parameter must be tuple or list. Got {config}'.format(config=self.config))
  ...
  def process_models(self, models_config):
    if not isinstance(models_config, (dict, list, tuple)):
      raise TypeError('"models" config for ADMIN_REORDER list item must be dict or list/tuple. Got %s' % repr(models_config))
    ordered_models_list = []
